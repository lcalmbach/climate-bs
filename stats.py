...
class Stats:
    ...
    def show_histogram(self, df):
        settings = {
            ...
            "height": 400,
            "x": "wert",
            "x_title": self.sel_par,
            "y_title": f"Anzahl {self.get_time_unit_plur()}",
        }
        settings[
            "title"
        ] = f"Histogramm {self.sel_par}, {self.sel_years[0]} - {self.sel_years[1]}"
        if self.type == StatType.MONTHLY.value:
            settings["title"] += f", Monat {self.sel_month}"
            if st.sidebar.checkbox("Zeige letzten Monat in Grafik"):
                value = self.data[
                    (self.data["datum"] == self.max_date)
    ...
    def show_heatmap(self, df):
        settings = {
            ...
            "y": "jahr:N",
            "color": "wert",
            "tooltip": ["parameter", "jahr:O", "monat", "wert"],
        }
        settings[
            "title"
        ] = f"Heatmap {self.sel_par}, {self.sel_years[0]} - {self.sel_years[1]}"
    ...
    def show_timeseries(self, df):
        settings = {
            ...
            "y": "wert",
            "x_title": "Jahr",
            "y_title": self.sel_par,
            "tooltip": ["datum", "parameter", "wert"],
        }
        ...
        settings[
            "title"
        ] = f"Zeitreihe {self.sel_par}, {self.sel_years[0]} - {self.sel_years[1]}"
        if self.type == StatType.MONTHLY.value:
            settings["title"] += f", Monat {self.sel_month}"
    ...
    def show_table(self, df):
        ...
        text = (
            text
        ) = f"""Alle Werte für Parameter `{self.sel_par}` in den Jahren {self.sel_years[0]} bis {self.sel_years[1]}."""
        if self.type == StatType.MONTHLY.value:
            text += f""" für den Monat {self.sel_month}."""
        if self.current_year in range(self.sel_years[0], self.sel_years[1] + 1):
            text += (
                f""" Das aktuelle Jahr hat Rang {current_year_rank} von {len(df)}."""
            )
    ...
    def show_menu(self):
        def get_time_series_data():
            if self.type == StatType.MONTHLY.value:
                df = self.data[
                    (self.data["monat"] == self.sel_month)
                    & (self.data["parameter"] == self.sel_par)
                ]
                df = df[
                    df["jahr"].isin(range(self.sel_years[0], self.sel_years[1] + 1))
                ]
            else:
                df = self.data[
                    (
                        self.data["jahr"].isin(
                            range(self.sel_years[0], self.sel_years[1] + 1)
                        )
                    )
                    & (self.data["parameter"] == self.sel_par)
                ]
        ...
        if self.type == StatType.MONTHLY.value:
            ...
            self.sel_month = st.sidebar.selectbox(
                "Monat", options=STAT_MONTHS, index=idx
            )
        self.sel_years = st.sidebar.slider(
            "Jahr", min_value=1921, max_value=2022, value=(1921, 2022)
        )
