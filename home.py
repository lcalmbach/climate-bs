...
class Home():
    ...
    def get_data(self):
        def add_temperature(df):
            temp = MonthlyAverage()
            df = df.join(temp.data[['Year', 'Month', 'Temperature']].set_index(['Year', 'Month']), on=['jahr','monat_id'], how='inner',  lsuffix='', rsuffix='_agg')
    ...
    def show_menu(self):
        ...
        (sel_year_from, sel_year_to) = st.slider(f'Vergleich {max_month_name}-Daten aktuell mit den {max_month_name} Werten der Jahre', min_value=1921, max_value=int(year-1), value = (1921, int(year) -1))
        ...
        df_last_month = df_last_month.rename(columns={'wert': f"{max_month_name} {year}"})
        ...
        df.style.format("{:.2f}")
        st.markdown(f"### Zusammenfassung Witterungserscheinungen im {max_month_name} {year}")
        st.write(f"Vergleich mit Mittelwert, Extrema und Standardabweichung der Jahre {sel_year_from} bis {sel_year_to} des gleichen Monats")
